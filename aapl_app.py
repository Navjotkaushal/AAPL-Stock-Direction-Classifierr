...
with st.sidebar:
    ...
    <div style='padding: 8px 0 20px'>
        <div style='font-family: Space Mono, monospace; font-size: 10px;
                    text-transform: uppercase; letter-spacing: 2px; color: #555577;'>
            Ticker
        </div>
        <div style='font-family: Space Mono, monospace; font-size: 32px;
                    font-weight: 700; color: #e0e0ff; margin-top: 4px;'>
            {TICKER}
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    n_iter = st.slider("Tuning Iterations", 10, 80, 40, disabled=not tune)
    ...
    st.markdown("""
    <div style='position: fixed; bottom: 24px; font-size: 10px;
                color: #333355; font-family: Space Mono, monospace;'>
...
if run_btn:
    ...
    try:
        with st.status("Running pipeline...", expanded=True) as status:
            ...
            st.write(f"✅ {X_train.shape[1]} features  |  Train: {len(X_train)}  Test: {len(X_test)}")
...
with tab_data:
    if st.session_state.df is not None:
        ...
        st.dataframe(
            df.tail(50).style.format(precision=2),
...
with tab_feat:
    if st.session_state.X_train is not None:
        ...
        c4.metric("Up Days (train)", f"{up_pct:.1f}%")
        ...
        with col_r:
            ...
            st.dataframe(
                X_train.describe().T.style.format(precision=4),
                use_container_width=True,
                height=280
            )
    else:
    ...
    for r in range(2):
        for c in range(2):
            ax.text(c, r, cm[r, c], ha="center", va="center",
                    color="white" if cm[r, c] > cm.max() / 2 else "#aaa", fontsize=15, fontweight="bold")
...
def make_imp_fig(model, name):
    ...
    bars = ax.barh(np.array(FEATURE_COLS)[idx], imps[idx], color="#55A868", alpha=0.85)
...
with tab_pred:
    if st.session_state.trained_models is not None:
        ...
        <div style='margin-bottom: 24px'>
            ...
            <div style='font-size: 13px; color: #555577; margin-top: 4px;
                        font-family: Space Mono, monospace;'>
        ...
        for col, (name, model) in zip(cols, trained_models.items()):
            prob      = model.predict_proba(latest)[0, 1]
            is_up     = prob >= 0.5
            direction = "⬆ UP"   if is_up else "⬇ DOWN"
            cls       = "pred-up" if is_up else "pred-down"
            conf_bar  = prob      if is_up else (1 - prob)
        ...
        votes_up   = sum(1 for _, m in trained_models.items()
                         if m.predict_proba(latest)[0, 1] >= 0.5)
        votes_down = len(trained_models) - votes_up
