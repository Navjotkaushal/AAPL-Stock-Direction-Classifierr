...
def run_pipeline(tune=False):
    ...
    try:
        ...
        if df.empty:
            raise ValueError("No data in DB. Run update manually before pipeline.")
        ...
        if not results["ohlc_clean"]:
            raise ValueError(f"OHLC violations: {results['ohlc_violations']}.")

        if results["has_nulls"]:
            raise ValueError(f"Nulls found: {results['missing_values']}.")

        if results["duplicate_dates"] > 0:
            raise ValueError(f"{results['duplicate_dates']} duplicate dates.")

        if results["suspicious_price_jumps"] > 0:
            print(f"WARNING: {results['suspicious_price_jumps']} suspicious jumps on "
        ...
        if tune:
            ...
            # extract just the model from (model, param_grid) tuple
